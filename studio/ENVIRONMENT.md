...
| **SessionStart** | `handoff.py --quiet` | no — it prints the other thread's open findings for this thread into context, so a handoff never depends on a human carrying a link (2026-09-22; L070) |
| **PostToolUse** on `Bash` | `thread-scope.py --tree` | no — but it reads the TREE, not the command, so every write technique lands here: a story file dirty on an environment branch is reported with the command to undo it |
| **PreToolUse** on `Bash` (a `git push`) | `thread-scope.py --push` | **yes** — an environment branch whose diff against `origin/main` carries an unwaived story path cannot push. The command check can be evaded; the diff cannot |
...
| `ack.py` + `studio/threads/ACKED.md` | **A suite so loud nobody reads it** | On 2026-09-23 the run printed 39 findings against 11 passes, most of them repeats from the previous week. A finding already on the handoff board now prints as a count; a finding the author has RULED needs his name and a date. A TRACKED acknowledgement dies the moment its row closes, so closing a row can never hide a finding. |
| `calibration.py` | **A gate firing on a wrong number** | The readers' romance level ran three points over the author's on ch 20 and ch 23. `targets-check` holds a chapter at two under target — so a chapter the author would call a 3 passes as a 6, laundered by a green check. This measures the error per instrument and states the sample size; under three paired readings it refuses to judge rather than sounding certain. |
| `export-book.py` | **Checking the source instead of the product** | Every check in the repo read the manuscript; none read the book. This assembles the text the reader gets and lints THAT. Pasted into KDP as-is, Book 1.1's raw manuscript would have shipped 95 lines of production headers. Also reports missing front and back matter, which the launch study says is where KU series money is made. |
| `cycle.py` + `studio/CYCLE.md` | **Breaking the release cycle** | The author: *"I always have one book finished when we launched the previous book… the biggest failure mode is getting out of this cycle."* Book N may not release while Book N+1 is incomplete — held at the [RELEASE] PR. One line at every SessionStart. Its LEAD lines watch the two quiet breaks: the book after next not started, and launch admin still open past two-thirds. |
| `comment-census.py --scorecard` | **The environment thread's own score** | Per chapter, the share of the author's comments that repeat a shape whose fix had already shipped. Not a comment count — new taste is the author writing the book. The first version read only PR-attributed notes and scored ch 23 at 0%; the three chat notes after #180 repeat MORE and POINT and make it 38%. |
| `order-lint.py` + `studio/threads/orders/` | **A finding becoming a revision pass** | The author, 2026-09-24: *"an anti-pattern is for us to continually look backwards at stuff that's already written."* An order must lead with the forward fix; a backward ask needs a mission-critical section and, over three chapters, gets a warning. Written the day O001's first draft turned a real measurement into a pass over seven finished chapters — by the thread whose job is to stop anti-patterns. |
| `guardrails.sh --opening` | **A cold opening nobody sees until the book is done** | The first three chapters measured the day they exist: the dial, the pressure curve, talk-to-body. Book 1.2's opening was findable at 8,000 words; nobody looked until 64,000. Three is the floor — one or two hold too little signal. |
| `proposal-lint.py` + `/propose` | **"Document it instead" rotting into a note nobody can act on** | The author, merging #183: *"Instead of making any changes just document your changes to any chapters so I can provide to my writer threads."* The lock stops the edit; this is what happens instead. A proposal names the file, quotes the current text and the replacement, and the lint verifies the quoted text is in that file **byte for byte and exactly once** — so the writer thread applies it by search-and-replace, and a proposal whose page has moved fails loudly instead of being applied to prose that changed. |
| `handoff.py` + `studio/threads/HANDOFF.md` | **A finding that needs a human to carry it** | Two threads, and the only route between them was the author pasting a PR link into the other conversation. That is the superfan failure's shape: a step nobody owns. Findings now arrive the way staleness does — printed at SessionStart, addressed by scope — and a row the AUTHOR raises to BLOCK holds that chapter's accept gate. An agent may raise a finding to FIX and argue; it may not hand itself a veto over the book thread's work. |
