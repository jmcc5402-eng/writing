# Book 1.2 — Wave 1 Directive (ch 1–4)

Issued 2026-08-30 by the showrunner, post-#105 (premise + outline
CANON; all 13 gate defaults locked — book2/DECISIONS.md B2-D02)
and post-storefront (copy CLEARED with two fixes —
notes/storefront-test-2026-08-30.md). First wave under the FULL
ratified stack (studio/RETRO-BOOK1.md §II+§III). Passes the
BRIEF AUDIT before any drafter fires; every manifest fact below
is a verbatim quote with file:line, per the amended item 1:
"Every manifest fact = a verbatim quote with file:line; a gloss
without its quote is a blocking finding."

## The conveyor for this wave

1. THIS directive → brief audit (continuity-keeper, BLOCKING).
2. 12 blind drafts — 3 candidates per chapter, cards dealt
   below; slot C every chapter is the JEWEL-FORGE (NEW 17).
3. Labels permuted + reading order rotated before panels
   (NEW 16); sealed permutation logged by the orchestrator.
4. 4 romance-reader-panel judgings (P1 binge posture rides —
   the 1.2 P-deck deal), warmth first.
5. Cross-batch canon sweep (OVERRIDES panels; sweeps graft
   packages too).
6. Staging order → stage on `campus/b2-ch01-04-wave` →
   lint battery (dialogue floor, dangling-edge awk, double-dash
   count, 7-word shingle dedup vs 1.1 accepted + semantic-wrap
   check at staging per NEW 21) → reading page → ONE author PR.

## Cards dealt (LRU; D6 rides every wave per NEW 18; D5 = the
## jewel-forge card, fixed to slot C per NEW 17)

| Ch | A | B | C (jewel-forge) |
|---|---|---|---|
| 1 | D4 minor-character competence | D7 open mid-motion | D5 weather-as-texture, briefed lyric-dense |
| 2 | D6 run past comfortable | D2 hands and objects | D5 lyric-dense |
| 3 | D3 short-declarative | D1 sound-forward | D5 lyric-dense |
| 4 | D6 run past comfortable | D4 minor-character competence | D5 lyric-dense |

Jewel-forge notice (in the C briefs): your candidate is scored
on GRAFT YIELD, not rank — write the lines only you would
write; density over safety. Cards shift emphasis, never
standards.
...
- Output: `book2/drafts/2026-08-30-wave1/ch0N-candidate-X.md`.
- BLIND: read the canon docs and voice refs listed here; do NOT
  read other candidates, panels, or drafts directories.
- No `[TK]` inside spoken lines or at emotional peaks; unknowns
  get record-or-soften.

## The leads (quote-locked; do not re-derive)
...
- STAGING CLAUSE (`studio/RETRO-BOOK1.md:155-158`, NEW 18): the
  mandated peak is "arrived-at, in motion, in the POV body,
  delayed past comfortable, never captioned."
- WARMTH ENGINE (`studio/RETRO-BOOK1.md:165-167`, NEW 19): "The
  warmth-engine sentence rides every brief verbatim ('hands
  doing the loving, mouth saying something else')."
- SERIES DIAL (`studio/RETRO-BOOK1.md:160-163`, NEW 19):
  "chorus buys the keep-reading (a 6), only lead-want buys the
  purchase (7+); every ensemble set-piece resolves into a
  two-person gesture."
- ANCHOR-LINE LAW (`studio/VISION.md:86-91`): "we need to stick
...
- CLOSENESS LAW from ch 1 (`studio/STYLE.md:164-173`): "A
  reaction beat per SCENE, not per chapter… one or two
  sentences where the POV character judges what just happened,
  in voice. Roughly forty words a scene"; "One private
...
- BANNED: "one beat" in any timing construction.
- BANNED: giving every speaker a punchline; write one flat line
...
- Rule 1 (`:169-174`): "DO run every grievance through a child.
  The realistic public register is performative concern: 'just
  worried about the kids,' 'asking for clarity on the
  rotation'…"
- Rule 4 (`:187-194`): "DO give the physician dogpile its
  documented shape: impatience with medical caution framed as
...
nightly-tab habit (echo pays 29)."
...
- He is on Boyd's facilities side OUT LOUD — likable,
  reasonable, said to someone; no authorial wink that he is
  wrong.
...
the bonfire's bravado, priced."

Conditions:
- The peak (the hit, the tent) runs the STAGING CLAUSE:
  arrived-at, in motion, in her POV body, delayed past
  comfortable, never captioned.
- Medical accuracy register: concussion protocol as an athletic
...
Introduces: Denny Gault's board voice (RH2 begins), Missy Gault
in person. Plants: the leads' registers aligned for the first
time."
...
- The midnight board thread runs Grapevine rules 1 and 4
  verbatim register ("just asking for clarity on the evaluation
  timeline… dressed as devotion"). No named accusation; the
  board is polite and it lands like weather.
- Trey stays offstage even here: a door, a chart, his parents'
  faces — never the boy.
...
Dialogue floor 15% (tool: `studio/tools/dialogue-lint.py`; no
quiet-band chapter dealt this wave — Q1's one quiet slot stays
banked); dangling-edge awk (`STYLE.md:48`); double-dash count;
7-word shingle dedup vs ALL 1.1 accepted chapters AND the other
wave-1 winners; semantic-wrap check (NEW 21); anchor-line count
= exactly 1 per chapter, chorus-voiced; swear count vs the
wave deal (≤1, Dan, ch 4); "never once" / "which was its own"
watch counts do not increase.
