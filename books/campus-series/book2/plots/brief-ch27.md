# Brief — Chapter 27: "The Hearing" (Aisha · Monday, February 1 — the review sits; climax one)

**Status: SET PIECE — three blind drafters (distinct variance cards,
same brief and addendum).** Climax one of two (B2-D33.1); BEATS "Grand
gesture → 27"; arc-docs 27, both leads: "The moment of change." Romance-
arc row 27, In 5 / Out 5: "The hearing: his name spent in public." The
card (`notes/cards/ch27-card.md`) and this brief are audited TOGETHER
by the keeper before a drafter launches. The author said "Defaults" on
the card's calls and the ladder call (AUTHOR-NOTES 292, 2026-09-24).

**TARGETS (the definition of done — `canon/TARGETS.md` row 27, copied;
CHANGED 2026-09-24 by the author: heat 4 → 5, AUTHOR-NOTES 293):**
Romance 8 · Heat 5 · Aisha 3 · Dan 3 · Wound 2 · Fun 1 · Town 3 ·
Menace 3 · Ends flat · Talk normal · Words 4200 · Pays Dan (words 3800 → 4200 at the fold: the author's two #197 asks added ~400, AUTHOR-NOTES 296–297).
On the AUTHOR'S scale: romance moves on CONFLICT between the leads.
Here the conflict is public and one-way: he spends his name on her in a
room of forty, and she has to let it count — the thing her false belief
exists to prevent (arc-docs 27, hers). Romance 8 is the testimony seen
from her chair, and the one count of their hands after. Heat 5 (0–8,
the charge scale; a CEILING — raised from 4 by the author: every panel
read the one deliberate private touch at 5, L105): the hallway after — his hand and hers,
bodies in it, four or more body sentences at the touch, no kiss, out of
anybody's sight (the terms). Aisha 3: TURNED — she does not stop him,
does not correct him, does not book it; she lets it count. Dan 3:
TURNED, seen only through her. Wound 2 (hers: defended in public by
someone she did not ask). Fun 1: one real laugh in the room. Town 3:
forty seats full; Earlene, Marisol, a trustee, Missy — the town acts.
Menace 3: Boyd MOVES — by evening the pledge is "re-sequenced" and the
extension is off the February agenda; Boyd himself never on the page.
Talk NORMAL (15–25%). Ends FLAT (held quiet). Pays Dan. Words 3800;
4370 the ceiling (±15%).

**Every number is the whole chapter (TARGETS definitions, L107).** No
reader sets a number from one line; the drafter does not write one line
to hit a number.

## MAIN MOVED — the rulings this chapter carries (read first; overrides the body)

1. **Climax one — he testifies, and it COSTS (B2-D33.1; L106).** Dan
   walks in unscheduled — NOT on the review's list (ch 26: "You're not
   on its list") — in the doorway, in the suit he hates, against the
   university's lawyer (ch 24, 26; unnamed) and against the carrot the
   reader heard in 26 ("your extension in February is close to a
   formality"). Aisha does not know the carrot; the READER does, and
   the lawyer half-standing is the reader's reminder. **How he gets the
   floor (keeper B5 — SETTLED for all three drafters; new canon,
   proposed at the PR):** the review sits in an open room; the reviewer
   says at the start, once, that the review will hear anyone it chooses
   to, at the reviewer's own discretion. Marisol asks and is heard; Dan
   asks from the doorway and is heard. No other procedure is invented.
   He says, in this order or close to it:
   - **the timeline (keeper B4 — narrowed to what the pages date):**
     the hold ran from November 28; her no-go (Thursday, December 17)
     came before the families' letter reached the athletic director
     (the Monday after Christmas, ch 22) and before the picture (January
     18, ch 24). NOT "before everything": the board's count (Dec 12) and
     Denny's draft (ch 6) came first;
   - **the pressure he put on her, and where it broke against her** —
     his own part, said plain, NAMED not dated by month (keeper N5): the
     Friday-check push (ch 2) and the escalation behind the athletic
     director's email of Tuesday, December 1 (ch 5);
   - **a rung for someone not Aisha (STATE; keeper N14):** he takes the
     pressure on himself so his staff are not in it — his staff did what
     he told them;
   - **"trading favors" is STRUCK before launch (keeper B3 —
     CONTRADICTION flagged to the author at the PR):** no page makes a
     favor-shaped charge except the relationship (ch 12, 23–24); a
     denial on the record breaks "nothing about them" and F-DAN-02, and
     would be a lie. If the laugh is his, it stays on the CALL itself —
     she cost him his season's best player and would do it again — with
     no word for any arrangement between them. Otherwise Delores's pad
     takes the laugh;
   - **the sentence, finished, VERBATIM (B2-D33.1):** "I said I'd have
     played him. That's exactly why the rule says I don't decide."
   - **then her name** — plainly, the whole of it, on the record: the
     gap the county heard on the fourteenth ("her protocol," ch 22) and
     on the twenty-sixth (the sound bite) closed (B2-D31.3).
   He says NOTHING about them (the terms; F-DAN-02). He does not ask
   her anything. He does not look for her approval before he speaks.
   In public he says "7," "my quarterback" or "the boy," never "Trey"
   (keeper N7). After he speaks: the reviewer thanks him and calls a
   recess, and Dan leaves the room first (keeper B7b); the review is
   NOT closed Monday — it sits again Wednesday morning, when Missy
   stands (outline 28; keeper N9).
2. **Aisha's turn — she lets it count (arc-docs 27, hers).** She does
   not stop him, does not correct him, does not take the record back
   and say it better. The pull to do all three is ON THE PAGE, in her
   body (she corrects people for a living; she has never once let a man
   pay for her — ch 26: "I'm not going to be the thing this cost you").
   The turn is that she sits still and lets it be paid. It settles
   NOTHING between them (B2-D33.5): public, not private.
3. **Her case — the chart wins without a speech (F1; B2-D31.1; the
   research note).** She delivers STANDING (ch 4:87–88), in the one good
   jacket she came to town with, NOT the parka (it is on the top box in
   the car, ch 25–26) (dossier-aisha 27). She says **"the patient"** for
   the whole of her testimony so the boy's name is not in the
   transcript; nobody in the room notices; it costs her the only warmth
   she has in that room (dossier-aisha 27 — her rooting moment).
   - **The reviewer's FIRST question (B2-D31.1):** why her ladder ran
     slower than other programs'. Fair, and the page lets it be fair
     (taste 23). Two fair phrasings in the research note §4; write your
     own.
   - **Her answer, and the ruled fact (the author, call 4 — "bad
     enough"):** the checklist's twenty-four hours is the least a step
     takes, not the most; at step two, minute fourteen, sixteen days
     out, he answered — his hand went up under the hood (ch 13:139–143)
     — and it was MORE than the checklist lets a physician ride through
     (not mild, not brief). So the stop was the checklist: step down,
     clock restarted (ch 13:151, 166–168), and past two weeks her own
     re-evaluation — the January re-eval under load (ch 21). **Birmingham
     is NOT her referral (keeper B1):** the Gaults booked it themselves
     (ch 14, 17, 21); what she did was send the whole chart on her own
     (ch 17:235). What Birmingham ANSWERED is not hers to say (B2-T04,
     28's). No symptom score as a number — write the fact without one.
   - **The chart (keeper B8):** the August baseline (on the page as
     imaging, ch 5:75, 281; if the studies are named, the Nov 28 and Dec
     3 dates, ch 13:269–272), the dated orders, the tape times (14 and
     7:59, ch 13), the re-eval (ch 21), Missy's seven o'clock pages —
     every rung dated. She answers for herself: she has no lawyer on any
     page (keeper N1).
   - **The records' custody (F1; ch 14, 25):** the annex's forty-one
     RECORDS boxes, locked at Mt. Nebo since December — not the
     clinic's numbered boxes, and Trey's paper chart is in neither (her
     office drawer, ch 25:227) (keeper N15); her count matches
     Delores Fountain-Mabry's pad; the pad does not leave the church
     (Delores, ch 25:287–289 — "the review can come to this hall and
     ask me for it"). This can be the chapter's laugh if Dan's is not.
   - **The rule, named once, no number (research note §3):** the
     team physician's authority over return-to-play is, in the NCAA's
     own words, "unchallengeable" and "autonomous" — "Independent
     Medical Care." Said by her or the reviewer; NEVER a bylaw number.
     Dan's "the rule" in MAIN MOVED 1 is this rule.
4. **A trustee on her side (B2-D31.4; one line, not a speech).** `[TK
   the trustee's name]` — what would this room be saying today if she
   had cleared him and he had gone down. One voice among the town's;
   the town is split, not one voice.
5. **Marisol's receipts — the decode (B2-T07 PAYS; F3; provider-
   partner row 27).** Marisol Pruett lays the eleven families' letter
   beside **the consultant's printed materials** — ch 10's "You'll see
   the phrase in the materials" (keeper B2; the survey notice and the
   brochure are on no page, STRUCK): one coined phrase in both —
   "proactive risk posture" (ch 10:129–135; ch 18:110–113). **How she
   holds both — ONE clause on the page (new canon, `[TK]` proposed at
   the PR):** the letter was organized on the board she moderates, so
   she has the families' own copy; the materials came to the public in
   the trustees' February agenda packet. The campaign
   shown manufactured; the families shown honest people, used (B2-D31.2).
   **The wire to Boyd stays UNPROVEN — said out loud ONCE, by Marisol,
   that she cannot prove who wrote it.** Boyd's name: at most once, in
   that sentence, or not at all. She is heard at the reviewer's
   discretion (MAIN MOVED 1); she moderates the room that campaigns
   against her own son (STAKES 60), her boy Wes a walk-on (registry 32).
6. **Missy in the room — RULED (call 2, default).** Missy Gault is in
   the forty, and Aisha sees Dan's words land on her: one or two
   physical tells, no reading of them (the notebook shut on her thumb,
   ch 21; her hand on her purse's clasp). Missy does NOT stand today —
   that is 28's, and HIS testimony is why (B2-D33.1). Aisha does not
   know what is in the purse (B2-T04 is untold). Denny: the drafter's
   call whether he is beside her; if so, one detail, no line — his
   refusal to sign (ch 18:114) is 28's. Trey is NOT in the room (keeper
   N8).
7. **The hallway after — RULED (call 3, default).** His hand and hers
   for one count, and NO WORDS at all (keeper B6 — the card wins). OUT
   OF SIGHT: the
   terms still hold two more days — nothing anybody can see till the
   first Wednesday in February (registry 114); caught in season, both
   fired that day (F-DAN-02) — so the page places it where nobody can
   see (a stairwell, a side corridor, a coat rack's corner — the
   drafter's, said). The page says (keeper B7c) that he left the room
   first at the recess, how she knows where he is with no words between
   them (she sees which way he goes), and why neither is seen by the
   lawyer or the forty (they go the other way — to the front hall, the
   coffee). Heat 5 (AUTHOR-NOTES 293): bodies in it, four or more body sentences; no kiss;
   both clear-headed. Then they are apart again.
8. **Boyd's money moves by evening (boyd-arc 27; game-stakes 27;
   F-AGD-01).** **The public source (keeper B7e — SETTLED; new canon,
   proposed at the PR):** the trustees' posted February agenda,
   amended late in the day — the pledge item "re-sequenced to a later
   sitting," the extension item struck. "Re-sequenced" was Boyd's word
   to Dan alone (ch 20:147), so the page's source is the posted agenda,
   not a mouth. No vote date anywhere. **The messenger:** Verna, who
   read it (the drafter says where Aisha is when Verna finds her). Say
   "February agenda," never "spring agenda" (keeper N10).
   She knows exactly what that cost him, in her body. BANNED here: any
   reading that her own job is saved by it — she knows the group rides
   on the pledge (ch 25:153), and this brief SAVES her doing that sum
   for 29 (the brief's own sequencing; keeper N11); the portal
   quarterback's flip (28's, on air).
9. **Where the turn signal went — RULED (call 1, default): Dan tells
   her in 29.** Nothing on this page says where he drove on the night
   of the twenty-eighth. She does not ask.
10. **Earlene's anchor (outline 27), in her words.** In the hallway
    before: all the doctor ever did here was her job, and today the
    county finds out if that is allowed. BANNED verbatim; fresh words.
11. **The ending (B2-D33).** ENDS FLAT, held quiet: the findings
    pending (Wednesday the third), his name spent, hers not yet
    answered. The acceptance is still typed and unsent (ch 25) — she
    does NOT send it here (28's), and she does NOT unpack (29's). The
    car stays loaded. NOT a decision.
12. **The body, not the talk (H007).** Talk is NORMAL, and a hearing is
    all talk — so the body carries more: chapter-lint TALK vs BODY
    under 2.0. Her body through his testimony is the chapter's spine.
13. **Heat — the new line (H009).** Body words on the page; off the
    page the act and explicit anatomy. Consent: both clear-headed.
14. **The revision closes every reader ask (conveyor item 6; L102).**

## PROOF OF DONE — campus 1.2 ch 27 (chapter-proof; filled at the end)

MECHANICAL (commands, output attached at the end)
  [ ] bash studio/tools/chapter-lint.sh manuscript/ch27.md → BANS clean; TOUCH SPAN ≥4 body sentences at the highest touch (the hallway); ≤2 questions ending in a period
  [ ] python3 studio/tools/dialogue-lint.py manuscript/ch27.md → normal band (15–25%)
  [ ] python3 studio/tools/bans.py manuscript/ch27.md → clean
  [ ] opening-check / ending-check → not a repeat
  [ ] python3 studio/tools/fact-check.py books/campus-series → clean
  [ ] ≤4370 words; SENTENCE SHAPE CV ≥ 0.60, p95 ≥ 33, at least one sentence over thirty (H001)
  [ ] TALK vs BODY under 2.0 (H007)
READERS
  [ ] author-proxy → notes/ch27-author-proxy-<A|B|C>-<date>.md (BEFORE the panel)
  [ ] romance-reader-panel → notes/ch27-panel-<A|B|C>-<date>.md (BLIND — card-blind copy)
  [ ] continuity-keeper → notes/ch27-keeper-<date>.md
  [ ] /chapter-score → notes/scores/ch27-score.md
MUST NOT HAVE CHANGED
  [ ] "I said I'd have played him. That's exactly why the rule says I don't decide." VERBATIM; her name after it
  [ ] ch 26 l.57–59's words NOT on this page (keeper N10)
  [ ] Dan does NOT ask her to stay; she does NOT send, withdraw or unpack (28, 29)
  [ ] the wire to Boyd unproven; Boyd never on the page
  [ ] Missy does not stand; Birmingham's answer unsaid
  [ ] "the patient," never Trey's name, in HER testimony
  [ ] no bylaw number; no symptom score number
  [ ] the hallway touch out of sight; no kiss; heat ≤ 5 (AUTHOR-NOTES 293)
  [ ] no new fact, name, date or object not on a page or in canon — `[TK]` it
VARIANCE
  [ ] three cards drawn (LRU, drafter deck; distinct) and logged; the keeper's, the proxy's and the panel's draws logged

## THE ROMANCE MOVE (first line)

Stage 5 held, In 5 / Out 5 (romance-arc row 27: "his name spent in
public"). The reader, angry at him at 25 and understanding at 26, sees
him pay at 27 — and sees HER let him, which is the harder thing for
her. Earned by: 22 ("her protocol"), 25 (the sound bite; "He had not
asked"), 26 (the carrot; "I'm not going to be the thing this cost you";
the turn signal). Spends: the sentence's second half; her name on the
record. Saves: the ask, the take-back and the unpacking (29); the coat
(29); Missy's purse (28).

## THE LEADS' SCENES (the floor)

ONE room together, publicly: the hearing (the longest scene; the
chapter's highest moment; slow down at his testimony — taste 16). ONE
private count: the hallway after. APART, WHY — said once, in her head:
the terms hold till Wednesday; nothing between them settled at her door
on Thursday.

## THE OPENING, SAID

Monday the first, the hallway outside the trustees' boardroom, before
the review sits — the date inside a sentence that does something else.
MID-MOTION on a thing none of the accepted openings used (… 24 rain on
a windshield, 25 a plate coming down, 26 the interview room). NOT a
pen, pitcher, legal pad, plate, counter, phone, door, or car.

## THE ARGUMENT, SAID

*Her record is airtight and it may not be enough: the question was
never whether she was right but whether a doctor who was right is
allowed to stay. The man who said he'd have played the boy walks in and
finishes the sentence, and it costs him the thing he was promised for
keeping quiet.* Between the leads: *she has spent her life refusing to
be the thing anybody pays for, and today somebody pays anyway, in
front of everyone, and she has to let him.*

## THE SCENES

1. **Morning — the hallway outside the trustees' boardroom** (the
   opening). The room seats forty (1.1 ch 28:18; the building `[TK]`)
   and every seat is full, the 1.1 echo unremarked. Earlene's anchor
   (MAIN MOVED 10). **How she got here (keeper B7a):** the loaded RAV4
   stays in Verna's gravel — a loaded car at the building would be read
   by the county (ch 25:434–435); Verna drives her in and does not come
   in. **The good jacket (keeper N2):** first time on a page — NOT the
   wool city coat ("a good coat," ch 7); the page says the wool coat
   stayed at the Court and the parka on the top box. **The lawyer:** a
   man in a gray suit with no folder (ch 24:81). The athletic director:
   in the room or not, the drafter says; if in, no line (keeper N4). Who is here, fast: Missy (and Denny?),
   Marisol, Delores or not, the lawyer, the reviewer's table. Dan is
   not here — she notes it and does not let herself look for him.
   Kinds 8, 9.
2. **The review — her case** (MAIN MOVED 3, 4). The reviewer's first
   question; her answer from the chart, standing; "the patient"; the
   forty-one boxes and Delores's pad; the rule named once. The
   trustee's one line. Kind 6 (her work as the want's weather).
3. **Marisol's receipts** (MAIN MOVED 5). The phrase side by side. The
   room turns. Unproven, said once. Kind 6.
4. **Dan, in the doorway** (MAIN MOVED 1, 2, 6; the longest beat;
   slow). The suit. The lawyer half stands. The reviewer lets him. The
   testimony, in his words (the list), the room's laugh, the finished
   sentence, her name. Her body the whole way through — the pull to
   stop him, and not stopping him. Missy's tells. Kinds 1, 5, 8.
5. **After — the hallway, out of sight** (MAIN MOVED 7). One count;
   bodies; no words that settle anything. Kinds 2, 5.
6. **Evening — the news reaches her** (MAIN MOVED 8, 11). From a person
   already on her pages. **Between (keeper B7d):** the session runs
   into the afternoon; she goes back to the Court; Verna finds her
   there near dark. The pledge re-sequenced; the extension gone.
   The car still loaded; the acceptance still unsent. ENDS FLAT, held
   quiet. Kind 8.

## THE AUTHOR'S READ (studio/AUTHOR-QUESTIONS.md; L056)

| Scene | MORE | WHO | CONFUSING | NOSE | POINT | SENSES | FUN |
|---|---|---|---|---|---|---|---|
| 1. The hallway | Romance felt 4: she does not look for him, and the page shows her not looking | Earlene: wants the county to find out if doing your job is allowed; Aisha: wants her record to be enough | What the review is (the one Monday the first, the findings Wednesday); why the room is full | Nobody says "big day"; the forty seats do it | The point: the town came | The radiator, the coats, the wet floor — the drafter's | Earlene's dryness |
| 2. Her case | Romance felt 3: she is alone at the table | The reviewer: wants the true answer, fair; the trustee: on her side, one line; Aisha: conflict 3 | Why her ladder was slower — answered in plain words, not a lecture | Nobody praises her; the dates do | The point: the chart is airtight — and it might not matter | Paper, her standing, her voice | The pad that doesn't leave the church |
| 3. The receipts | Romance felt 3: she thinks of him once | Marisol: wants the room to see it was handed to them; the families: honest, used | What the phrase is and why it matters (the letter, the notice, side by side) | Nobody says "Boyd did it"; the phrase does | The point: the campaign was manufactured; the wire unproven | Two sheets under a lamp or projector | — |
| 4. Dan | Romance felt 9: her body through his testimony | Dan: pays; the lawyer: wants silence; Missy: listening; Aisha: lets it count | Why he is not on the list; how he gets the floor; what the finished sentence means | Nobody says "he chose her"; the price does | The point: he pays, publicly, and she lets him | The doorway, the suit, his voice, her hands | The room's one laugh |
| 5. The hallway after | Romance felt 8: one count, bodies | Both: nothing settled | Where they are and why nobody sees | Nobody says anything that settles it | The point: public is not private | His hand, the cold wall, breath | — |
| 6. Evening | Romance felt 6: the cost, in her body | The messenger: one line, their own want | What "re-sequenced" means, in plain words | Nobody says "he did it for you" | The point: he paid; she still has not answered | The car, the boxes, the dark | — |

## ROMANCE BEATS (one per scene, at least three, two kinds)

Scene 1 — kinds 8, 9. Scene 2 — kind 6. Scene 3 — kind 6. Scene 4 —
kinds 1, 5, 8. Scene 5 — kinds 2, 5. Scene 6 — kind 8. The proxy and the
panel on the author's anchors.

## REVERSAL — who loses what

Dan loses the extension, the carrot, and the county's easy story
about him (PAYS Dan). Aisha loses her rule that nobody pays for her —
she lets it happen. Boyd loses the campaign's cover and moves his money
the same day. The families lose the letter's authority, not their good
faith.

## ARC BEATS

**Aisha (arc-docs 27):** she lets it count — being defended in public
by someone she did not ask. **Aisha (dossier 27):** standing, the good
jacket, "the patient." **Dan (arc-docs 27; dossier 27, seen):** the
doorway, the suit, her name on the record. **Boyd (boyd-arc 27):**
named once at most, never proven; the pledge re-sequenced.
**Marisol:** the receipts. **Earlene:** the anchor. **Missy:** listening.
**The trustee:** unnamed in the prose (keeper N13).

## STAKES ON THE PAGE (taste 22 amended — BLATANT; `canon/STAKES.md`)

| Who | Locked stake carried | The drop (whose eye, what) |
|---|---|---|
| Aisha | on leave; the findings Wednesday the third; the offer typed, unsent, answer Saturday the sixth (ch 25); the terms till Wednesday | her head, once: what the findings decide |
| Dan | the extension on the February agenda (F-AGD-01) and the carrot (ch 26); his staff's jobs ride on it (D31.5); caught in season, both gone that day (F-DAN-02) — a HINT only here | the lawyer standing; the evening's news |
| Missy | Birmingham in her purse, untold (B2-T04) — NOT said here | her hand on the clasp, seen, not read |
| Marisol | moderates the room that campaigns against her own son (STAKES 60) | her one sentence of what she can't prove |
| Earlene, the trustee, the reviewer, the lawyer | none locked | — |
| Boyd | the pledge; the partner (B2-D13) | the pledge re-sequenced, by evening |
| The building (D30.4 — never on the page without its price) | if the pledge is named, its price with it, fresh words | once, if at all |

## THE STAKES, THIS CHAPTER'S

Two days to the findings; five to her answer; the terms two more days;
his extension on the February agenda until tonight.

## COUPLE LINE

None. His words are to the room; hers to the room. The hallway has NO
words (keeper B6).

## END REGISTER: FLAT (held quiet)

26 ended on a button in his truck; 25 down at Verna's step; 24 down at
her office door. Not a storm door, not a car loaded as the last image,
not alone under a light, not a phone, not a door. The drafter's image;
a held breath, not a decision.

## ROOTING FOR

**Aisha:** "the patient" all day so the boy is not in the transcript —
seen by nobody. **Dan:** the price paid with the carrot in his pocket —
seen by her.

## NAMING, FULL LIST

Her POV: "Aisha" in narration; "Dan" in her head; "Coach Merritt" in
the room's mouths. "the reviewer" (`[TK]`); "the university's lawyer"
(unnamed); "Earlene" (Earlene Tatum, the front-row grandmother);
"Marisol" (Marisol Pruett); "Missy Gault"; "Denny" if present;
"Delores" (Delores Fountain-Mabry) if present or named; the trustee
`[TK]`. "Trey" in HER head only — "the patient" in her mouth. "Rex
Boyd": at most once, in Marisol's unproven sentence. "the athletic
director," never "the AD."

## FURNITURE, CITED

The trustees' boardroom, forty seats (outline 27; `[TK the building]`);
the good jacket (dossier-aisha); the parka on the top box (ch 25–26);
the loaded RAV4 (ch 25); minute fourteen, 7:59, the cap (ch 13, 15);
the January re-eval (ch 21); Missy's pages and notebook (ch 4, 15, 21);
the forty-one boxes and Delores's pad (ch 25); "proactive risk posture"
(ch 10, 18); "her protocol" (ch 22); the sound bite (ch 25–26); the
suit he hates (outline 27).

## BANS AND BUDGETS

- Sentences END: ordinarily under thirty; at most three "and"s. THE
  TAIL (H001): one or two sentences run past forty.
- ch 26 l.57–59's words (keeper N10; line numbers per the ch 27
  audit, N6): "the call had never been a coach's to make"; "It was the
  doctor's. It was hers."; "A coach always wanted to play the boy." The
  sound bite at l.54 MAY be quoted as the record.
- "Stay" asked by anybody; the send; the withdrawal; the unpacking
  (28, 29).
- Earlene's outline words verbatim ("today we find out if that's
  allowed"); the research note's phrasings verbatim; "the review can
  come to this hall" verbatim (Delores's, ch 25).
- A bylaw number; a symptom score; Birmingham's answer; the portal
  quarterback; the partner contract saved.
- BANNED: RECENT.md as of today; studio/lessons/bans.txt; "in this
  room"; "somebody's"; "never once"; "one beat"; "plainly" more than
  once; "electricity"; "the AD"; every thank-you shape (L042); a face
  that "did nothing" (L050); the empty chair; "said, to the <object>";
  heart-then-stomach; ch 24:270–272's "something went across her face…
  steady again" (panel, ch 26); neck heat, throat, pulse more than twice
  each.
- The clock between scenes on the page (L053).
- Names, not "he," where two men share a paragraph (L045).

## THE EPIGRAPH

A parent's eye, no plot, light. Under a handle unused by 21–26 (NEVER
Glenda Whitlow). Under 35 words. NOT the review.

## TASTE (the entries this chapter risks)

23 (fair on both sides — the reviewer's question is fair); 20 (the touch
has a body); 16 (slow down at his testimony); 1 (why he gets the floor;
what re-sequenced means); 21 (who Marisol, Earlene, the trustee are);
22 (stakes blatant); 8; 15 (rooted for — her stillness must read as
strength, not passivity); 7 (not her alone for 60% — she is in a room
of forty); 13 (the card).

## THE CARD'S CALLS — RULED at default (the author, "Defaults," 2026-09-24; AUTHOR-NOTES 292)

1. Where the turn signal went: Dan tells her in 29.
2. Missy is in the room watching Dan speak.
3. The hallway after: his hand and hers for one count.
4. The minute-fourteen headache was more than the checklist tolerates.

## AUDIT ADDENDUM — ch 27 card + brief (continuity-keeper, card E4, 2026-09-24) — BLOCK, then corrected in the body above

B1 Birmingham is the Gaults' second opinion, booked by them (ch 14, 17,
21); she sent the whole chart on her own (ch 17:235) — not her referral
(MAIN MOVED 3). B2 the survey notice and the brochure are on no page —
the second document is ch 10's "materials," and how Marisol holds both
is one clause of proposed new canon (MAIN MOVED 5; the card). B3
"trading favors" answers a charge no page makes except the relationship
— STRUCK before launch; a CONTRADICTION with arc-docs 27 flagged to the
author (MAIN MOVED 1). B4 the no-go did not precede "everything" —
narrowed to the letter reaching the athletic director and the picture
(MAIN MOVED 1). B5 the review's procedure SETTLED once for all three
drafters: an open room; the reviewer hears whom the reviewer chooses
(MAIN MOVED 1, 5). B6 the hallway has no words (MAIN MOVED 7; COUPLE
LINE). B7 the gaps between scenes said: how she arrives (Verna drives
her; the loaded car stays in the gravel), the recess and Dan leaving
first, how they find each other unseen, the afternoon to evening, the
agenda amended as the public source and Verna the messenger (scenes 1,
6; MAIN MOVED 1, 7, 8). B8 "not scans" struck — August imaging is on
the page (MAIN MOVED 3; the card).

NOTES N1–N16 applied: she answers for herself (N1); the good jacket
and the wool coat (N2); forty seats is 1.1 ch 28:18 (N3); the lawyer's
gray suit, the athletic director in or out (N4); the pressure named, not
dated (N5); the ban's lines l.57–59 (N6); "7" / "my quarterback" in
Dan's public mouth (N7); Trey not in the room, Denny's refusal 28's
(N8); the record not closed Monday (N9); "February agenda" (N10); the
partner sum saved by the brief's own sequencing (N11); STAKES 60's
wording (N12); the trustee unnamed (N13); a rung for his staff (N14);
two sets of boxes (N15); taste 8 added (N16).

NEW CANON (recorded at the fold): the suit he hates; the good jacket on
a page; the review's session rule (open room; the reviewer's
discretion); the transcript; the consultant's materials in the
trustees' February packet and the families' copy of the letter on the
board; the trustee `[TK]`; Missy's purse on the page; the hallway spot;
the agenda amended Feb 1 — pledge re-sequenced to a later sitting,
extension struck (F-AGD-01 scope note); Verna the messenger; "the
patient" in her testimony.

## VERDICT: PASS — B1–B8 corrected in the body above (B3 struck and flagged to the author at the PR); the notes applied
