...
_Started 2026-07-27, implementing recommendation 7 of
`notes/critique-2026-07-26.md`: one honest row per book — what was
actually earned ON THE PAGE, not what the bible aspires to. Sources:
`concept/premise.md` (40-book arc), `series-bible/story-engine.md`,
`characters/characters.md`, the Book 1 canon manuscript
(`manuscripts/book-01-hawaii/manuscript.md`), its thread map
(`manuscripts/book-01-hawaii/THREADS.md`), and the unadopted rewrite
`drafts/2026-07-26/` tagged **(v2 draft)**. Update this file every
time a book's draft changes what is true on the page._

---

## S-thread index (series-level threads)

Format: ID | name | type — arc so far / OWED. Book-local touchpoints
...
- **S02 — The agency and "the woman" (the parents' boss)** | knowledge
  born pre-canon (characters.md: "a woman they often mention") →
  B1: zero on-page mentions. OWED: a name and a face, timing [TK] —
...
- **S05 — The destination chain** | plot
  B1 ends: next assignment Tokyo, Japan — stated twice (ch12: Kekoa's
  footage request; "next consulting job would take them to Japan").
  RULES: every book ends with a hint of the next destination
  (story-engine §1); each book = new city, ~3-month stay by default
...
- **S07 — Jenny & Jacob (the worst-moment engine)** | motif
  ...
  twins un-grounded); Jacob's unknowing assists (ch3 "pieces of a
  map"; ch7 documentation/provenance lecture). New on-page facts to
  fold into characters.md: Peru dig documentation "last year" (ch7);
  current Indonesia project (ch3); Jenny and Erin grew up in Colorado
...
- **S08 — The pets (Blazer key-feature rotation; Moonbeam debt)** |
  ...
  scent chain cracks the case — B1 T04). Moonbeam: zero appearances
  in Book 1. OWED: Moonbeam's role (open question, premise.md +
  STATUS.md); do not invent cat scenes retroactively. RULES: describe
  the dog in Chapter 1 of every book; a pet mention every book; pets
  are a KEY plot feature every few books.
- **S09 — The online class & the other consultant kids** | knowledge
  born characters.md (rotating classmates "each with some cool
  attribute"; other consultant couples "with the same secret?") → B1:
  online classes appear as texture (ch4, ch7, ch9) but no classmate is
...
Cells state what is true ON THE PAGE of the canon manuscript. Where a
...
- **Lesson learned:** [TK — which twin unstated.]
- **Next-destination hint:** [TK — Book 3 city undecided.]
- Carried from Book 1 (per the outline): Andrew's drone/gear lobby +
  Kekoa's Tokyo-footage request; secondary-plot candidates include the
  phone lobby (S03/B1 T12) and Blazer travel/quarantine (S08).

### Books 3–40 — no rows yet

Rows are added when a book has an outline. Arc gates to audit at each
boundary: Arc 2 (11–20) "genuinely capable" twins must be backed by
